...
def get_fresh_token():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"⚠️  Failed to refresh token: {response.status_code}")
            return None
...
def load_progress():
    ...
    if PROGRESS_FILE.exists():
        with open(PROGRESS_FILE, 'r') as f:
            return json.load(f)
    return {"completed_tasks": [], "failed_tasks": [], "failed_details": {}, "last_task_id": None}
...
def get_all_tasks():
    ...
    while True:
        ...
        if response.status_code != 200:
            print(f"❌ Failed to fetch tasks (page {page}): {response.status_code}")
            print(response.text)
        ...
        if isinstance(data, dict) and 'results' in data:
            ...
            print(f"  📄 Fetched page {page}: {len(tasks)} tasks (total: {len(all_tasks)})")
            ...
            print(f"  📄 Fetched page {page}: {len(data)} tasks (total: {len(all_tasks)})")
...
def generate_prediction_for_task(task_id):
    ...
    try:
        ...
        if response.status_code != 200:
            error_msg = f"Failed to fetch task {task_id}: {response.status_code}"
            try:
        ...
        if ml_response.status_code != 200:
            error_msg = f"ML backend failed for task {task_id}: {ml_response.status_code}"
            try:
                error_detail = ml_response.json()
                error_msg += f" - {error_detail}"
            except:
                error_msg += f" - {ml_response.text[:200]}"
            print(f"  ❌ {error_msg}")
            return False, error_msg
        
        predictions = ml_response.json()
        if not predictions.get('results'):
            print(f"  ⚠️  No predictions returned for task {task_id}")
            return True, "No detections"  # Not a failure, just no detections
        ...
        if create_response.status_code not in [200, 201]:
            error_msg = f"Failed to save prediction for task {task_id}: {create_response.status_code}"
            try:
                error_detail = create_response.json()
                error_msg += f" - {error_detail}"
            except:
                error_msg += f" - {create_response.text[:200]}"
            print(f"  ❌ {error_msg}")
            return False, error_msg
        ...
        error_msg = f"Exception processing task {task_id}: {str(e)}"
        print(f"  ❌ {error_msg}")
        return False, error_msg

def main():
    """Main execution"""
    print("🚀 Label Studio Batch Prediction Generator")
    print("=" * 60)
    ...
    try:
        ...
        if health.status_code != 200:
            print("❌ ML Backend is not responding!")
            return
    ...
    if completed or failed:
        print(f"📌 Resuming from previous run: {len(completed)} completed, {len(failed)} failed")
    ...
    print(f"⏱️  Estimated time: {estimated_hours:.1f} hours ({estimated_seconds/60:.0f} minutes)")
    print()
    ...
    for i, task in enumerate(remaining_tasks, 1):
        ...
        try:
            success, message = generate_prediction_for_task(task_id)
            
            if success:
                print(f"  ✅ Success!")
                completed.add(task_id)
                progress['completed_tasks'] = list(completed)
                # Remove from failed_details if it was there
                if str(task_id) in failed_details:
                    del failed_details[str(task_id)]
            else:
                print(f"  ❌ Failed!")
                failed.add(task_id)
                progress['failed_tasks'] = list(failed)
                # Store failure reason
                failed_details[str(task_id)] = {
                    "image": image_name,
                    "error": message,
            ...
            progress['failed_details'] = failed_details
            ...
            print(f"  ⏱️  Avg: {avg_time:.1f}s/task, ETA: {eta_minutes:.0f} minutes")
            print()
    ...
    if failed:
        ...
        for task_id in sorted(failed):
            ...
            if task_id_str in failed_details:
                detail = failed_details[task_id_str]
                print(f"  - Task {task_id} ({detail.get('image', 'unknown')}): {detail.get('error', 'Unknown error')}")
            else:
                print(f"  - Task {task_id}")
        print("\n📋 Detailed failure log saved in prediction_progress.json")
        print("You can manually review these tasks in Label Studio.")
